...
import { PriceRangeFilter } from "@/components/payment/price-range-filter";
import { DateRangeFilter } from "@/components/payment/date-range-filter";
import { StatusFilter } from "@/components/payment/status-filter";
import { SearchBar } from "@/components/search-bar";
import { FilterTabs } from "@/components/payment/payment-filter-tab";
...
const StatusBadge = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 px-3 py-2 rounded-full text-xs font-medium ${config.bgColor} ${config.textColor} ${className}`}
    >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-2 mb-2">
              <h3 className="font-semibold text-gray-900 text-sm sm:text-sm truncate">
                {purchase.eventName}
              </h3>
            ...
            {purchase.paymentStatus === PAYMENT_STATUS.FAILED &&
              purchase.reason && (
                <div className="mt-2 flex items-start gap-2 p-2 bg-red-50 rounded text-xs">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                {purchase.items.map((ticket, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center py-2 px-3 bg-white rounded border text-xs"
                  >
                    <div>
                ...
                <div className="flex justify-between items-center pt-2 border-gray-200 font-semibold">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              {purchase.paymentStatus === PAYMENT_STATUS.PENDING && (
                <button className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors text-xs font-medium">
                  Complete Payment
                </button>
              )}
              {purchase.paymentStatus === PAYMENT_STATUS.FAILED && (
                <button className="flex-1 bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition-colors text-xs font-medium">
                  Retry Payment
                </button>
              )}
              {purchase.paymentStatus === PAYMENT_STATUS.SUCCESS && (
                <button className="flex-1 bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors text-xs font-medium">
                  View Details
                </button>
              )}
              <button className="flex-1 bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-200 transition-colors text-xs font-medium">
                Contact Support
              </button>
            </div>
          </div>
      ...
      <div className="w-full mx-auto">
        ...
        <div className="mb-6">
          <FilterTabs
            ...
            purchases={mockPurchasesWithDetails}
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={expandAll}
              className="flex items-center gap-2 px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 border border-gray-300 rounded-md hover:bg-gray-50"
            >
