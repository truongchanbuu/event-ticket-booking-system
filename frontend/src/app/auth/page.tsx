...
const EventHubAuth = () => {
  ...
  const [organizerStatus, setOrganizerStatus] = useState<APPLY_STATUS>(
    APPLY_STATUS.NONE
  );
  ...
  // Hiển thị LoadingPage khi đang check user profile
  if (userLoading) {
    return (
      <LoadingPage
  ...
  // Nếu đã đăng nhập (userProfile != null), show UI apply organizer
  if (userProfile) {
    if (!showApplyOrganizer && !skipApply) {
      const orgStatus =
                ...
                <ApplicationStatus
                  ...
                  onRetry={
                    organizerStatus === APPLY_STATUS.REJECTED
                      ? () => {
                          setOrganizerStatus(APPLY_STATUS.NONE);
    ...
    // Nếu đã đăng nhập nhưng không cần apply organizer, có thể return null hoặc loading tuỳ ý
    return <LoadingPage message="Redirecting..." subMessage="Please wait" />;
  }

  // Nếu chưa đăng nhập (userProfile === null), show signin/signup
  if (userProfile === null) {
    return (
  ...
  // Trường hợp fallback (không nên xảy ra)
  return (
    <LoadingPage
      message="Checking your profile..."
      subMessage="Please wait a moment"
