...
const app = express();
const TICKET_PORT = process.env.TICKET_PORT || 3001;
...
if (!serviceAccountPath) {
  ...
  console.error(
    'Please ensure your .env file is in the project root (D:\\GitHub\\event-ticket-booking-system\\ticket\\.env)',
  );
  console.error(
    'and contains the line: FIREBASE_KEY_PATH=./config/your-key-file.json (with your correct key file name).',
  );
...
try {
  ...
  console.log(
    `Attempting to load service account key from: ${absoluteServiceAccountPath}`,
  );

  // Read and parse the JSON file asynchronously using fsPromises
  const serviceAccountContent = await fsPromises.readFile(
    absoluteServiceAccountPath,
    'utf8',
  ); // <--- IMPORTANT FIX: Use fsPromises
  ...
  if (error.code === 'ENOENT') {
    ...
    console.error(
      `Error: Service account key file not found at "${absoluteServiceAccountPath}".`,
    );
    console.error(
      'Please ensure the file exists and FIREBASE_KEY_PATH in your .env is correct.',
    );
  } else if (error.name === 'SyntaxError') {
    console.error(
      `Error: Failed to parse service account key JSON file at "${absoluteServiceAccountPath}". It might be corrupted or not valid JSON.`,
    );
  } else {
    // Catch-all for other errors during Firebase init
    console.error(
      'Other Firebase Admin SDK initialization error:',
      error.message,
    );
  }
...
app.get('/', (req, res) => {
  res.status(200).json({
    message: 'Ticket Booking Backend Service is running!',
    status: 'healthy',
  });
});
...
app.post('/api/auth/create-test-user', async (req, res) => {
  try {
    ...
    if (error.code === 'auth/email-already-exists') {
      ...
      res
        .status(500)
        .json({ message: 'Failed to create test user', error: error.message });
    }
  }
});
  ...
  try {
    ...
    if (!eventName || !eventDate || !numTickets || numTickets <= 0) {
      return res.status(400).json({
        message:
          'Event name, date, and number of tickets are required and must be valid.',
      });
    }
...
app.get('/api/bookings/my', authenticateToken, async (req, res) => {
  try {
    ...
    bookings.sort(
      (a, b) =>
        (b.bookedAt?.toDate().getTime() || 0) -
        (a.bookedAt?.toDate().getTime() || 0),
    );
...
app.use((req, res, next) => {
  res
    .status(404)
    .json({ message: 'Not Found: The requested API endpoint does not exist.' });
});
