...
describe("PUT /:userID - Update user info", () => {
    let app;
    let userID;

    beforeAll(async () => {
        app = await createApp();
        userID = "useridthatdoesnotexist123";
    ...
    it("✅ should update user info successfully", async () => {
        const res = await request(app).put(`/${userID}`).send({
            username: "updated",
        ...
        expect(updatedDoc.data()?.username).toBe("updated");
        expect(updatedDoc.data()?.phoneNumber).toBe("0987654321");
    });
