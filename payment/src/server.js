...
async function bootstrap() {
  try {
    ...
    server = app.listen(PORT, () => {
      rootLogger.debug(`🚀 Payment service running on port ${PORT}`);
    ...
    const gracefulShutdown = async (signal) => {
      ...
      if (server) {
        await new Promise((resolve, reject) => {
          server.close((err) => {
            if (err) {
              rootLogger.error('❌ Error closing HTTP server:', err);
              return reject(err);
            }
            rootLogger.debug(
    ...
    console.error('❌ Failed to start the Payment service:', error);
    process.exit(1);
  }
}
